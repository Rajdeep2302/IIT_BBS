...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $conn->prepare("SELECT regid FROM admin WHERE regid = ?");
    ...
    $stmt = $conn->prepare("SELECT RegId FROM admin WHERE `email` = ? OR `phone` = ?");
    ...
    $stmt = $conn->prepare("INSERT INTO admin (`RegId`, `name`, `email`, `phone`, `password`) VALUES (?, ?, ?, ?, ?)");
    $stmt->bind_param("sssss", $registrationID, $fullName, $email, $phone, $hashedPassword);

    if ($stmt->execute()) {
        //set cookie
        setcookie("adminID", $registrationID, time() + (86400 * 30), "/"); // 86400 = 1 day
